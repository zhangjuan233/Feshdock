"""Module to parse the feshdock program command line options"""
...
def get_lightdock_structures(input_file):
    ...
    if input_file_extension == DEFAULT_LIST_EXTENSION:
        ...
        if os.path.exists(lightdock_structure):
            ...
            raise FeshdockError(f"Structure file {lightdock_structure} not found")
    return file_names
...
class SetupCommandLineParser(object):
    """Parses the command line of feshdock_setup"""

    def __init__(self, input_args=None):
        parser = argparse.ArgumentParser(prog="feshdock_setup")
...
class CommandLineParser(object):
    ...
    def __init__(self, input_args=None):
        parser = argparse.ArgumentParser(prog="feshdock")
